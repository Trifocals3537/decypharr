...
class DecypharrUtils {
    ...
    createToast(message, type = 'success', duration = null) {
        ...
        toastElement.className = `alert ${alertTypeClass[type]} shadow-lg mb-2`;

        const content = document.createElement('div');
        content.className = 'flex items-start gap-3';

        const icon = document.createElementNS(svgNamespace, 'svg');
        icon.setAttribute('class', 'w-6 h-6 shrink-0');
        icon.setAttribute('fill', 'currentColor');
        icon.setAttribute('viewBox', '0 0 20 20');
        const iconPath = document.createElementNS(svgNamespace, 'path');
        iconPath.setAttribute('fill-rule', 'evenodd');
        iconPath.setAttribute('clip-rule', 'evenodd');
        iconPath.setAttribute('d', iconPaths[type]);
        icon.appendChild(iconPath);

        const messageContainer = document.createElement('div');
        messageContainer.className = 'flex-1';
        const messageElement = document.createElement('span');
        messageElement.className = 'text-sm whitespace-pre-line';
        messageElement.textContent = String(message ?? '');
        messageContainer.appendChild(messageElement);

        const closeButton = document.createElement('button');
        closeButton.type = 'button';
        closeButton.className = 'btn btn-sm btn-ghost btn-circle';
        closeButton.setAttribute('aria-label', 'Close notification');
        closeButton.addEventListener('click', () => this.closeToast(toastId));
        const closeIcon = document.createElementNS(svgNamespace, 'svg');
        closeIcon.setAttribute('class', 'w-4 h-4');
        closeIcon.setAttribute('fill', 'none');
        closeIcon.setAttribute('stroke', 'currentColor');
        closeIcon.setAttribute('viewBox', '0 0 24 24');
        const closePath = document.createElementNS(svgNamespace, 'path');
        ...
        content.append(icon, messageContainer, closeButton);
